...
export class HtmlUpravitelj {
	...
	public async dokumentacija(req: Request, res: Response): Promise<void> {
		try {
	...
	public async dodavanjeOsoba(req: Request, res: Response): Promise<void> {
		try {
			...
			console.error(
				"Greška pri učitavanju stranice za dodavanje osoba:",
				error
			);
			res
				.status(500)
	...
	public async osobe(req: Request, res: Response): Promise<void> {
		try {
	...
	public async detalji(req: Request, res: Response): Promise<void> {
		try {
	...
	prijava: RequestHandler = async (req, res) => {
		const sesija = req.session as RWASession;
		if (sesija && sesija.korime) {
			console.log(
				`Korisnik ${sesija.korime} već ima aktivnu sesiju. Preusmjeravanje na /profil.`
			);
			res.redirect("/profil");
			return;
		}
		...
		if (req.method === "POST") {
			...
			try {
				const recaptchaResponse = await fetch(
					`https://recaptchaenterprise.googleapis.com/v1/projects/rwakmisic22-1736185312315/assessments?key=6Lfc268qAAAAAEfFooq6pSI4k_Xn8aDILGoCUkxB`,
					{
						method: "POST",
				...
				if (
					...
					console.error(
						"Invalid reCAPTCHA token properties:",
						recaptchaData.tokenProperties
					);
					greska = `Neuspješna reCAPTCHA verifikacija. Razlog: ${
						recaptchaData.tokenProperties.invalidReason || "Nepoznato"
					}`;
					const stranica = await this.ucitajStranicu("prijava", greska, req);
					res.send(stranica);
					return;
			...
			const korisnik = await this.servisKlijent.prijaviKorisnika(
				korime,
				lozinka
			);
			if (korisnik) {
	...
	async adminStranica(req: Request, res: Response) {
		try {
			const stranice = [
			...
			if (adminStranica && nav) {
				...
				if (sesija.uloga && sesija.uloga !== "gost") {
					odjavaLink = `<a href="/odjava">Odjava (${
						sesija.korime || "Korisnik"
					})</a>`;
				...
				nav = nav
					.replace(
						...
						`<a href="/prijava" ${prijavaStyle}>Prijava</a>`
					)
					.replace(
						'<a href="/registracija">Registracija</a>',
						`<a href="/registracija" ${registracijaStyle}>Registracija</a>`
					)
	...
	async filmoviPretrazivanje(zahtjev: Request, odgovor: Response) {
		const stranica = await this.ucitajStranicu(
			"filmovi_pretrazivanje",
			"",
			zahtjev
		);
		odgovor.send(stranica);
	...
	private async ucitajStranicu(
		nazivStranice: string,
		poruka = "",
		zahtjev?: Request
	) {
		let stranice = [
			this.ucitajHTML(nazivStranice),
			this.ucitajHTMLNavigacija(zahtjev?.session as RWASession),
		];
	...
	private async ucitajHTMLNavigacija(sesija?: RWASession): Promise<string> {
		let navigacija = await this.ucitajHTML("navigacija");

		if (sesija && sesija.korime) {
			navigacija = navigacija.replace('<a href="/prijava">Prijava</a>', "");
			navigacija = navigacija.replace(
				'<a href="/registracija">Registracija</a>',
				""
			);
		}

		return navigacija.replace(
			"#odjavaLink#",
			sesija?.korime ? `<a href="/odjava">Odjava (${sesija.korime})</a>` : ""
		);
	}
	...
	private async ucitajDokumentaciju(
		req: Request,
		...
		try {
			...
			const baseUrl =
				req.hostname === "spider.foi.hr"
					? `http://spider.foi.hr:12203`
					: `http://localhost:12222`;

			if (uloga === "gost") {
				const navLinks =
					nav.match(/<a href="[^"]+">[^<]+<\/a>(?!\s*--)/g) || [];
				nav = navLinks.filter((link) => !link.includes("/odjava")).join("");
				nav = `<nav>${nav}</nav>`;
			} else {
				const korisnikPodaci = await fetch(
					`${baseUrl}/servis/korisnici/${korime}`
				)
					.then((response) => {
						if (!response.ok) {
							throw new Error(
								`Greška pri dohvaćanju podataka: ${response.status}`
							);
						}
				...
				if (korisnikPodaci?.zahtjevZaPristup === "pending") {
					...
					nav = navLinks
						.filter((link) =>
							dozvoljeniLinkovi.some((dozvoljeni) => link.includes(dozvoljeni))
						)
						.map((link) => link.trim())
						.join("");
			...
			dokumentacijaStranica = dokumentacijaStranica.replace(
				"#navigacija#",
				nav
			);
	...
	private async ucitajDodavanjeOsobe(
		req: Request,
		res: Response<any, Record<string, any>>
	): Promise<string> {
		try {
			const stranice = [
				ds.readFile(
					path.join(__dirname(), "./html", "dodavanjeOsoba.html"),
					"utf-8"
				),
				this.ucitajHTML("navigacija"),
			];
	...
	async profil(zahtjev: Request, odgovor: Response) {
		const sesija = zahtjev.session as RWASession;
