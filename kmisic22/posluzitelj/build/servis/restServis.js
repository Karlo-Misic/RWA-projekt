...
export class RestServis {
    ...
    postOsoba(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(405).send({ greska: "Zabranjena metoda" });
    }
    putOsoba(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(405).send({ greska: "Zabranjena metoda" });
    }
    deleteOsoba(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(405).send({ greska: "Zabranjena metoda" });
    }
    ...
    async addOsoba(zahtjev, odgovor) {
        odgovor.type("application/json");
    ...
    getFilmoveZaOsobu(zahtjev, odgovor) {
        odgovor.type("application/json");
    ...
    postFilmoveZaOsobu(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(405).send({ greska: "Zabranjena metoda" });
    }
    putPoveziOsobuSaFilmovima(zahtjev, odgovor) {
        odgovor.type("application/json");
        const osobaIdParam = zahtjev.params["id"];
        if (!osobaIdParam) {
            odgovor
    ...
    deleteVezuOsobeSaFilmovima(zahtjev, odgovor) {
        odgovor.type("application/json");
        const osobaIdParam = zahtjev.params["id"];
        if (!osobaIdParam) {
            odgovor
        ...
        this.sdao
            ...
            .catch((greska) => {
            console.error("Greška prilikom brisanja veze osobe s filmovima:", greska);
            odgovor
                .status(400)
                .send({ greska: "Greška prilikom brisanja veze osobe s filmovima." });
        });
    }
    getFilmove(zahtjev, odgovor) {
        odgovor.type("application/json");
        ...
        const stranica = parseInt(stranicaParam, 10) || 1;
        const datumOd = datumOdParam
            ? parseInt(datumOdParam, 10)
            : undefined;
        const datumDo = datumDoParam
            ? parseInt(datumDoParam, 10)
            : undefined;
        if (isNaN(stranica) || stranica < 1) {
            odgovor
                .status(422)
    ...
    deleteFilmAkoNemaVeza(zahtjev, odgovor) {
        odgovor.type("application/json");
        const filmIdParam = zahtjev.params["id"];
        if (!filmIdParam) {
            odgovor
    ...
    getZadnjaOsoba(zahtjev, odgovor) {
        this.sdao
            .dajSveStranica(1)
            .then((osobe) => {
            if (osobe.length > 0) {
