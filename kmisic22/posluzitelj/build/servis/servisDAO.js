...
export class ServisDAO {
    baza;
    constructor() {
        const bazaPath = path.resolve(__dirname, "..//..//podaci/RWA2024kmisi22_servis.sqlite");
        if (!fs.existsSync(bazaPath)) {
            console.log("Baza ne postoji, kreiram novu...");
        }
        this.baza = new Baza(bazaPath);
    }
    async dajSveStranica(stranica) {
    ...
    async obrisiOsobuISveVeze(osobaId) {
        const deleteVezeSql = `
			DELETE FROM uloga
			WHERE id_osobe = ?;
		`;
            ...
            try {
                ...
                for (const filmId of filmovi) {
                    ...
                    if (vezaRezultat[0] && vezaRezultat[0].broj > 0) {
                        console.log(`Veza između osobe ${osobaId} i filma ${filmId} već postoji.`);
                    }
                    else {
    ...
    obrisiVezuOsobeSaFilmovima(osobaId, filmovi) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                console.error("Greška prilikom brisanja veze između osobe i filmova:", greska);
                reject(greska);
            }
        });
    }
    dohvatiFilmove(stranica, datumOd, datumDo) {
        ...
        const sql = `${osnovniSql} ${uvjeti.length > 0 ? `WHERE ${uvjeti.join(" AND ")}` : ""} LIMIT 20 OFFSET ?`;
        const offset = (stranica - 1) * 20;
    ...
    obrisiFilmAkoNemaVeza(idFilma) {
        ...
        return new Promise((resolve, reject) => {
            try {
                const rezultat = this.baza.dajPodatke(provjeriSql, [idFilma]);
                if (rezultat && rezultat[0] && rezultat[0].broj > 0) {
                    reject(new Error(`Film s ID-om ${idFilma} ima povezane veze i ne može biti obrisan.`));
                    return;
                }
    ...
    async dodajSliku(nazivSlike, osobeId = 190) {
        try {
            const maxIdResult = await this.baza.dajPodatkePromise("SELECT MAX(id) as maxId FROM slike", []);
            const maxId = Array.isArray(maxIdResult) &&
		  ...
		  `;
            ...
            await this.baza.ubaciAzurirajPodatke(sql, podaci);
            console.log(`Naziv slike '${nazivSlike}' uspješno spremljen u bazu s ID-om ${noviId}.`);
            return true;
        }
        catch (err) {
    ...
    async brojFilmovePoDatumu(datumOd, datumDo, offset = 0, limit = 20) {
        let uvjeti = [];
        let parametri = [];
        if (datumOd) {
        ...
        const whereClause = uvjeti.length > 0 ? `WHERE ${uvjeti.join(" AND ")}` : "";
        const sqlBroj = `SELECT COUNT(*) AS ukupno FROM filmovi ${whereClause}`;
        const rezultatBroj = (await this.baza.dajPodatkePromise(sqlBroj, parametri));
        const sqlFilmovi = `SELECT naslov FROM filmovi ${whereClause} LIMIT ? OFFSET ?`;
        const filmoviParametri = [...parametri, limit, offset];
        const rezultatFilmovi = (await this.baza.dajPodatkePromise(sqlFilmovi, filmoviParametri));
        return {
