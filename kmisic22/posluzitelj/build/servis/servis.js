...
function pripremiPutanjeResursTMDB() {
    ...
    server.get("/servis/tmdb/nasumceFilm", restTMDB.dohvatiNasumceFilm.bind(restTMDB));
...
function pripremiPutanjeResursKorisnika() {
    ...
    server.get("/servis/dohvatiKorisnike", restKorisnik.getKorisnike.bind(restKorisnik));
        ...
        try {
            ...
            if (korisnik) {
                res.status(200).json({
                    ...
                    spol: korisnik.spol,
                    zahtjevZaPristup: korisnik.zahtjevZaPristup === "zahtjevNijePoslan"
                        ? "Nema pristup"
    ...
    server.put("/servis/korisnici/:korime/status", async (req, res) => {
        ...
        try {
            ...
            if (!["approved", "pending", "zahtjevNijePoslan", ""].includes(noviStatus)) {
    ...
    server.post("/servis/osoba/:id/film", restServis.postFilmoveZaOsobu.bind(restServis));
    ...
    server.get("/servis/zadnja-osoba", (req, res) => restServis.getZadnjaOsoba(req, res));
...
function UploadSLike() {
    const mapaZaUpload = path.resolve(__dirname(), "../../../angular/projects/kmisic22/src/app/uploads");
    ...
    server.post("/upload", (req, res) => {
        ...
        req.on("end", async () => {
            const dijelovi = tijelo
                ...
                .split(granica)
                .filter((dio) => dio.trim() !== "--");
            const filePart = dijelovi.find((dio) => dio.includes('Content-Disposition: form-data; name="image"'));
            if (!filePart) {
            ...
            let originalName = originalNameMatch && originalNameMatch[1]
                ? originalNameMatch[1]
                : `slika-${Date.now()}`;
            if (!path.extname(originalName)) {
                ...
                const mimeType = mimeTypeMatch && mimeTypeMatch[1] ? mimeTypeMatch[1].trim() : null;
                const ekstenzija = mimeType && ekstenzije[mimeType] ? ekstenzije[mimeType] : ".bin";
            ...
            try {
                ...
                console.error(`[ERROR] Greška pri spremanju datoteke ili baze: ${greska}`);
    ...
    server.use("/uploads", express.static(mapaZaUpload, {
        setHeaders: (res) => {
