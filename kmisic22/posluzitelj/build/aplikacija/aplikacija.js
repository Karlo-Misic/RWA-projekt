...
function pripremiPutanjeDodavanjeOsoba() {
    ...
    const htmlUpravitelj = new HtmlUpravitelj(konf.dajKonf().jwtTajniKljuc, portRest);
    server.get("/dodavanjeOsoba", fetchUpravitelj.provjeriPristup(["admin", "moderator", "obican"]), htmlUpravitelj.dodavanjeOsoba.bind(htmlUpravitelj));
}
function pripremiPutanjeOsobe() {
    const fetchUpravitelj = new FetchUpravitelj(konf.dajKonf().jwtTajniKljuc);
    const htmlUpravitelj = new HtmlUpravitelj(konf.dajKonf().jwtTajniKljuc, portRest);
    server.get("/osobe", fetchUpravitelj.provjeriPristup(["admin", "moderator", "obican"]), htmlUpravitelj.osobe.bind(htmlUpravitelj));
}
function pripremiPutanjeDetalji() {
    const fetchUpravitelj = new FetchUpravitelj(konf.dajKonf().jwtTajniKljuc);
    const htmlUpravitelj = new HtmlUpravitelj(konf.dajKonf().jwtTajniKljuc, portRest);
    server.get("/detalji", fetchUpravitelj.provjeriPristup(["admin", "moderator", "obican"]), htmlUpravitelj.detalji.bind(htmlUpravitelj));
}
function pripremiPutanjeAdmin() {
    const fetchUpravitelj = new FetchUpravitelj(konf.dajKonf().jwtTajniKljuc);
    const htmlUpravitelj = new HtmlUpravitelj(konf.dajKonf().jwtTajniKljuc, portRest);
    server.get("/admin", fetchUpravitelj.provjeriPristup(["admin"]), htmlUpravitelj.adminStranica.bind(htmlUpravitelj));
}
